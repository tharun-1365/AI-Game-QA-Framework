...
| PB-003 | Hazard on golden path (regression) | Platform 3 walking surface, x18.5 (`PB_003_SpikeOnGoldenPath`) | Third spike `Spike_C` at (18.5, 2.5) — clean platform 3 carries no hazard | A clean-level golden run replayed here dies mid-route | Scored evidence: the run's own `RunEnded {outcome:"SpikeDeath"}` (+ `PlayerDied {cause:"spike"}`). Ground-truth regression (context, not the scored path): a clean golden `Success` replayed here flips to `SpikeDeath` — validation.json v3 `outcomeMatch:false` | **HazardOracle (exists, M5-D)** — scored directly from the recorded `SpikeDeath` outcome (same path as PB-001), via M6-C. No oracle consumes the original-vs-replay `outcomeMatch`; that comparison is ground truth, not the detector | Major | planted (M6.A) | pending |
